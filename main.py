...
RegionNames = region_codes['region_slug'].iloc[0:100]
#RegionNames = ["waikato"]

print(f'Getting property IDs for: {", ".join(RegionNames)}')

property_IDs_to_update = id.get_all_properties_by_region(sitemap_df, RegionNames, headers, user_agents)

print(f'Beginning full load for: {", ".join(RegionNames)}')
...
regionIDs = region_codes['regionID'].iloc[0:100]
